...
9. **Тестирование развертки просисходит на виртульной машине с ubuntu.** На основном устройстве создается виртуальная машина с ubuntu, куда скачиивается проект из github. Далее через терминал происходит запуск sudo ./start.sh
...
packages/
  ...
  db/                  Prisma schema, миграции, сиды (каталог предметов — item-templates-defaults.ts)
...
`pnpm db:seed` (`packages/db/src/seed/index.ts`) сейчас прогоняет: `rule-config`, расы, группы, типажи НПС, первичный admin из ENV, dev-users, **каталог предметов** (`item-templates-defaults.ts`).
Каталог предметов: статические TS-данные в `packages/db/src/seed/item-templates-defaults.ts` (112 шаблонов, `ITEM_TEMPLATES` + `seedItemTemplates`) — ранее парсились из `Gob_markets.csv`, теперь перенесены в TS ради единообразия сидов и самодостаточного деплоя (нет внешнего файла → нечему теряться при clone). Десятичные хранятся строками (Prisma приводит к Decimal). Натуральный ключ upsert — `name` («Категория Тир N»). Сид идемпотентен и не каскадит на персонажей.
